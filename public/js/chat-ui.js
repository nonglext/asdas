 ...
 *  Собирается узлами: никнеймы и ID пользователей никогда не попадают в HTML как разметка. */
function renderChatWelcome(box, { nick, avatar, group, sub, strong }) {
  const w = document.createElement('div');
  w.className = 'chat-welcome';

  const avEl = document.createElement('div');
  avEl.className = 'chat-welcome-av' + (group ? ' group-av' : '');
  avEl.setAttribute('aria-hidden', 'true');

  const title = document.createElement('div');
  title.className = 'chat-welcome-title';
  title.textContent = nick == null ? '' : String(nick);

  const subEl = document.createElement('div');
  subEl.className = 'chat-welcome-sub';
  subEl.textContent = sub == null ? '' : String(sub);
  if (strong) {
    const b = document.createElement('b');
    b.textContent = String(strong);
    subEl.append(b, document.createTextNode('.'));
  }

  w.append(avEl, title, subEl);
...
async function openChat(id) {
  ...
  const loaded = await loadConversationHistory(box, `${BACKEND_URL}/api/messages/${encodeURIComponent(state.me.id)}/${encodeURIComponent(id)}`, { nick: f.nickname, avatar: f.avatar, sub: 'Это начало личной переписки с ', strong: '@' + f.id }, m => appendMsg(m, 'messages', false), 'chat', requestSeq, unreadBefore);
...
async function openGroupChat(groupId) {
  ...
  const loaded = await loadConversationHistory(box, `${BACKEND_URL}/api/groups/${encodeURIComponent(groupId)}/messages`, { nick: g.name, group: g, sub: 'Это начало общего разговора. Всё, что здесь пишут, видят все участники.' }, m => appendGroupMsg(m, false), 'groupChat', requestSeq, unreadBefore);
...
function updateGroupVoiceBar(groupId) {
  ...
  if (members) {
    ...
    (call?.participants || []).forEach(peerId => {
      ...
      avatar.title = nick;
      // Через renderAv, а не через background-image: строка из БД не попадает в CSS.
      renderAv(avatar, nick, member?.avatar || null);
 ...
 * PROFILE: edit own
 * ==========================================================================*/
/* Аватар применяется ТОЛЬКО по кнопке «Сохранить изменения».
   До сохранения выбранный файл живёт здесь и показывается локальным blob-предпросмотром. */
const pendingAvatar = { file: null, previewUrl: null, remove: false };

function releaseAvatarPreview() {
  if (pendingAvatar.previewUrl) {
    try { URL.revokeObjectURL(pendingAvatar.previewUrl); } catch (e) {}
  }
...
function syncAvatarPendingUi() {
  ...
  if (remove) remove.hidden = pendingAvatar.remove || (!state.me?.avatar && !pendingAvatar.file);
  $('edit-profile-modal')?.classList.toggle('avatar-dirty', dirty);
}

/** Сбросить незасохранённый выбор и вернуть аватар из профиля. */
function discardPendingAvatar() {
  const had = !!pendingAvatar.file || pendingAvatar.remove;
...
window.discardPendingAvatar = discardPendingAvatar;
...
/* Выбор файла: только локальный предпросмотр, ни одного запроса на сервер. */
on('avatar-input', 'change', e => {
  ...
  if (!ALLOWED_AVATAR_TYPES.includes(avatarMime) && !avatarExt) {
  ...
  pendingAvatar.previewUrl = URL.createObjectURL(file);
  renderAv($('edit-avatar'), state.me.nickname, pendingAvatar.previewUrl);
  syncAvatarPendingUi();
  input.value = '';
});
...
on('btn-avatar-reset', 'click', () => {
  if (discardPendingAvatar()) showTransientNotice('Изменения аватара отменены');
});

/** Загружает выбранный файл. Возвращает URL аватара или бросает ошибку. */
async function uploadPendingAvatar() {
  const file = pendingAvatar.file;
  const ext = String(file.name || '').toLowerCase().match(/\.(jpe?g|png|webp|gif)$/)?.[1] || 'png';
  const formData = new FormData();
  formData.append('avatar', normalizeImageFile(file, `avatar.${ext}`), file.name || `avatar.${ext}`);
  const res = await authFetch(BACKEND_URL + '/api/upload/avatar', { method: 'POST', body: formData });
  if (!res.ok) {
    const d = await safeJson(res);
    throw new Error(d.error || 'Не удалось загрузить аватар');
  }
  const data = await res.json();
  if (!data?.avatar) throw new Error('Сервер не подтвердил загрузку аватара');
  return data.avatar;
}
...
on('btn-save-profile', 'click', async () => {
  ...
  await withButtonBusy($('btn-save-profile'), 'Сохранение…', async () => {
    try {
      // 1. Сначала аватар: если загрузка не удалась, остальные поля не трогаем.
      let nextAvatar = state.me.avatar;
      if (pendingAvatar.file) nextAvatar = await uploadPendingAvatar();

      // 2. Затем текстовые поля (и снятие аватара, если его попросили убрать).
      const payload = { nickname, status, bio };
      if (pendingAvatar.remove) payload.avatar = null;
      ...
      state.me.avatar = pendingAvatar.remove ? null : (u.avatar ?? nextAvatar ?? null);
      ...
      if (!(e instanceof AuthError)) showTransientNotice(e?.message || 'Ошибка сети');
    }
  });
});
