...
function closeAllModals() {
  // Незасохранённый аватар не должен «пережить» закрытие диалога.
  try { window.discardPendingAvatar?.(); } catch (e) {}
 ...
 * ==========================================================================*/
/**
 * Белый список источников картинки. Значение приходит из БД или сокета, поэтому
 * проверяется явно: загрузки бэкенда, http(s) и локальный blob: для предпросмотра.
 * javascript:, data:, vbscript: и прочее отбрасываются — в src такое не попадёт.
 */
...
function avatarSrc(url) {
  ...
  if (u.startsWith('blob:')) return u;                 // локальный предпросмотр до сохранения
  if (UPLOAD_PATH_RE.test(u)) return BACKEND_URL.replace(/\/+$/, '') + u;
  try {
    const parsed = new URL(u, BACKEND_URL);
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') return '';
    if (parsed.origin === new URL(BACKEND_URL).origin && !UPLOAD_PATH_RE.test(parsed.pathname)) return '';
    return parsed.href;
  } catch { return ''; }
}
...
function renderAv(el, nickname, avatarUrl) {
  if (!el) return;
  el.textContent = '';
  const safeSrc = avatarSrc(avatarUrl);
  if (safeSrc) {
